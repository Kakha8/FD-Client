...
public class LockboxRestController {
    ...
    private ResponseEntity<StreamingResponseBody> download(Long fileId,long revision,LockboxObjectStorage.ArtifactType type) throws Exception {
        ...
        StreamingResponseBody body=out->{try(InputStream input=result.inputStream()){input.transferTo(out);}};
        return ResponseEntity.ok().contentType(MediaType.parseMediaType(result.contentType())).contentLength(result.size())
                .cacheControl(CacheControl.noStore()).header(HttpHeaders.CONTENT_DISPOSITION,ContentDisposition.attachment().filename(result.fileName()).build().toString()).body(body);
    }
